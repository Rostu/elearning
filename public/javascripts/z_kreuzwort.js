...
var Word = new Array("WELTWIRTSCHAFT", "ROBOTER", "HANDYSTRAHLEN", "ZUKUNFT", "LENKEN", "RADAR", "WISSENSCHAFTLER", "ATEMBERAUBEND", "VERDOPPELN", "TAPETE");
var raetsel = [];


var Clue = new Array("umfasst alle internationalen Wirtschaftsbeziehungen",
    "künstlicher Maschinenmensch oder Automat, der ferngesteuert oder programmiert bestimmte Tätigkeiten ausführt, die sonst von Menschen verrichtet werden",
    "Energie, die sich geradlinig von einem heute alltäglichen Kommunikationsgerät fortbewegt",
    "die noch bevorstehende, die kommende Zeit, Zeitraum nach dem gegenwärtigen Zeitpunkt",
    "etw. in eine bestimmte Richtung bewegen, führen, oder lenken",
    "in der Funkmeßtechnik verwendetes Verfahren (auch Gerät) zur Ermittlung und Standortbestimmung von Objekten",
    "jemand, der sich mit der (organisierte) Form der Erforschung, Sammlung und Auswertung von Kenntnissen beschäftigt, Forscher",
    "etw. oder jemand mit begeisternden, faszinierenden, oder fesselnden Eigenschaften",
    "etw. um die gleiche Anzahl, Größe, Menge, das gleiche Maß vermehren, verzweifachen",
    "bedruckte oder bemalte Wandbekleidung aus verschiedenem Material, z.B. aus Papier oder Stoff");
...
function init() {
    ...
    var horizontal = [];
    var vertical= [];
    for(i=1;i<=Word[0].length;i++){
        for(y=1;y<=Word[0].length;y++){
            horizontal.push(vertical);
        }
        raetsel.push(horizontal);
        horizontal = [];
    }

    x=0;
    y=0;
    raetsel.forEach(function(horizontal){
        var div = jQuery('<div/>', {
            class: 'row',
            id: y
        });
        horizontal.forEach(function(char){
            var div2 = jQuery('<div/>', {
                class: 'char_box',
                id: "["+y+"]["+x+"]"
            });
    ...
    var cw = new Crossword(["WELTWIRTSCHAFT", "ROBOTER", "HANDYSTRAHLEN", "ZUKUNFT", "LENKEN", "RADAR", "WISSENSCHAFTLER", "ATEMBERAUBEND", "VERDOPPELN", "TAPETE"], ["","","","","","","","","",""]);
    var grid = cw.getGrid(50)
    r=0;
    e=0;
    grid.forEach(function(row){
        row.forEach(function(elem){
            if(elem != null){
                $(".char_box[id='["+r+"]["+e+"]']").css("background-color","white");
                console.log(elem.char + "["+r+"]["+e+"]");
            }
            e++;
        })
        e=0;

/**
 * Created by JuBre on 20.01.2016.
 */
    ...
    function myFunction() {
        ...
        for (i = 0; i < this_list.length; i++) {
            ...
            if (this_list[i].attr("id") === ui_draggable_list[i].attr("id")) {
                this_list[i].css("background-color", "#80FF80");
                this_list[i].css("color", "black");
                this_list[i].css("font-weight", "bold");
                count++;
                raisepoints();
        ...
        //Message
        if (count === 7) alert("Super! Du hast alles richtig gemacht! Auf zur nächsten Aufgabe!");
        else if (count > 3 && count < 7) alert("Sehr gut! Du hast die meisten Wörter richtig eingesetzt. Schau dir die Hinweise an.  Weiter so!");
        else alert("Das war schon ein guter Anfang. Schau dir die Hinweise an. Probiere es weiter!");
